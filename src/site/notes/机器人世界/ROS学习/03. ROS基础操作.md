---
{"dg-publish":true,"permalink":"/机器人世界/ROS学习/03. ROS基础操作/"}
---
...
+ src：代码空间，存放所有[[机器人世界/ROS学习/01. ROS1架构学习#^25df91\|ROS功能包的源码文件]]；
...
# debug版本
colcon build --cmake-args -DCMAKE_EXPORT_COMPILE_COMMANDS=ON -DCMAKE_BUILD_TYPE=Debug --packages-select pointcloud_test
```
...
```
# 调整日志等级（全局debug）
ros2 run pointcloud_test pointcloud_intra_process --ros-args --log-level debug

# 调整日志等级（指定特定node name）
ros2 run pointcloud_test pointcloud_intra_process --ros-args --log-level pointcloud_publisher:=debug
```