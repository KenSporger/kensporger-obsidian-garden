---
{"dg-publish":true,"permalink":"/机器人世界/ROS学习/02. MAC安装ROS/"}
---
...
![Pasted image 20260209173655.png](/img/user/%E6%9C%BA%E5%99%A8%E4%BA%BA%E4%B8%96%E7%95%8C/ROS%E5%AD%A6%E4%B9%A0/assets/Pasted%20image%2020260209173655.png)
...
由于ROS安装在Docker中，本地没有ROS环境，导致VScode/Cursor打开各种语法报错。解决方案：
+ 安装 ROS2（VSCode）/Robot Developer Extensions for ROS 2（Cursor）
+ VSCode安装Dev Containers

Cursor连接步骤：
1. **Cursor 左下角**，有一个 **绿色的远程连接图标**（<> 形状），点击安装“Dev Containers”；
2. 点击它 → 弹出菜单选择 **附加到运行中的容器 (Attach to Running Container)**，选择ROS所在容器；
3. 在容器的环境中，安装必要的插件：C++、Python、Clangd、CodeLLDB。
4. 使用`colcon build --cmake-args -DCMAKE_EXPORT_COMPILE_COMMANDS=ON`编译ROS2项目，会在build目录下生成`compile_commands.json`，执行`ln -s build/compile_commands.json compile_commands.json`链接到根目录，重启Cursor，Clangd就能解析这个文件语法跳转高亮才正常（高亮颜色通过切换cursor主题来配置）。
5. 修改settings文件，否则一些C++头文件找不到。
```cpp
{
  // Ensure cpptools uses the workspace compilation database (merged by colcon/tools script).
  "C_Cpp.default.compileCommands": "${workspaceFolder}/compile_commands.json",
  "C_Cpp.default.compilerPath": "/usr/bin/c++",
  "C_Cpp.default.cppStandard": "c++17",

  // If you prefer clangd for navigation, you can disable cpptools IntelliSense
  // and keep it only for debugging/tasks.
  "C_Cpp.intelliSenseEngine": "disabled",

  // Helpful when working in containers: prefer the compile_commands database.
  "C_Cpp.default.configurationProvider": "",
  "cmake.ignoreCMakeListsMissing": true,

  // Force clangd to query the real compiler for system include paths
  // (libstdc++ headers like <chrono> live under /usr/include/c++/<ver>/).
  ...
  // Enable semantic highlighting (required for types like std::string to be colored).
